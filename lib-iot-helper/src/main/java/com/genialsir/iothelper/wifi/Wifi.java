...
//        wifi.isConnected = wifi.SSID.equals(connectedSSID) && ipAddress > 0;
        ...
        wifi.ip = wifi.isConnected ? String.format("%d.%d.%d.%d",
                (ipAddress & 0xff),
                (ipAddress >> 8 & 0xff),
                (ipAddress >> 16 & 0xff),
                (ipAddress >> 24 & 0xff)) : "";
    ...
    public static IWifi create(boolean connectedStatus, WifiInfo wifiInfo) {
        if (TextUtils.isEmpty(wifiInfo.getSSID())) {
            return null;
        }
        Wifi wifi = new Wifi();
        wifi.name = wifiInfo.getSSID().replaceAll("\"", "");
        wifi.SSID = wifiInfo.getSSID();
        wifi.isConnected = connectedStatus;
        wifi.isEncrypt = true;
        wifi.encryption = "";
        wifi.ip = wifi.isConnected ? String.format("%d.%d.%d.%d",
